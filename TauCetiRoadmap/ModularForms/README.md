...
- **(a′) The local `p`-adic comparison.** Relate the integral double-coset theory above to its
  local `p`-adic form through two milestones. First, identify the `p`-integral part with the
  genuinely `p`-adic
  ...
  local algebra `ℤ[T_{p,1}, …, T_{p,n−1}, T_{p,n}^{±1}]` (Rhodes–Shemanske). These are
  requirements of this layer: the `n = 2` theory is complete in AINTLIB; general `n` has exactly
  the two named steps recorded in (a); and the completion criterion is
  `R_p_isPolynomialRing` sorry-free at general `n` together with these two local-comparison
  milestones. The [weight-two quaternionic-inner-form
  roadmap](../WeightTwoAutomorphicForms/README.md) consumes only the `n = 2` comparison of
  double-coset bases and structure constants. It does not consume the general-`n` polynomial
  presentation, localization, or Satake theory.
...
- The Hecke algebra in this roadmap is the classical double-coset ring of (a)–(b). Its adelic
  reformulation is **out of scope** here: the weight-two quaternionic-inner-form roadmap consumes
  the `n = 2` ring and local comparison, then builds the projective adelic action on its own side.
...
- **Layer 2** (the Hecke ring at `GL_n`; the `n = 2` congruence theory and the action) → Layer 0, Mathlib's abstract Hecke ring (fallback: AINTLIB). The general-`n` block remains a requirement of this roadmap; the weight-two quaternionic-inner-form roadmap consumes only its `n = 2` double-coset handoff.
...
| Layer 2 `GL_n` block | migrate; general `n`: two named steps open (**source gap**); the weight-two quaternionic-inner-form roadmap consumes only the `n = 2` handoff |
