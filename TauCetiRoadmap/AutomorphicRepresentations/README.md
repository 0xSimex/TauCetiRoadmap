...
This roadmap builds enough reusable Lean infrastructure to state the following conjecture without
...
> `Dˣ(𝔸_F)`. There are a number field `E`, Hecke polynomials `P_v ∈ E[X]` at all but finitely many
> finite places `v` of `F`, and a semisimple weakly compatible family of continuous
...
polynomial, and “weakly compatible,” is in scope here.
...
schemes is therefore not a prerequisite for the target statement.
...
4. Compatibility is weak, at almost all finite places. The coefficient representations live over
   `Q̄_p` and are indexed by embeddings `E → Q̄_p`; they are not required to descend to `GL₂(E_λ)`.
...
- **SIGN-OFF A — cuspidality and one-dimensional representations.** The Zulip formulation says
  “cohomological automorphic representation,” without saying “cuspidal.” For the usual theorem on
  `GL₂` one takes cuspidal representations. Characters and Eisenstein constituents should give
  reducible systems, but including them changes both the spectrum and the statement. This roadmap
  uses the cuspidal form because it is the clean theorem needed by the FLT application. A reviewer
  should either confirm this restriction or specify the larger class and its reducible conclusion.
- **SIGN-OFF B — cohomological normalization.** For even-rank groups, the passage between
  ...
  chosen algebraic coefficient system, central character, Satake normalization, and reciprocity
  convention produce exactly that polynomial.
- **SIGN-OFF C — strength of compatibility.** Carayol proves a strictly compatible system with
  ...
  characteristic polynomials. The summit below intentionally states the weak version. If Kevin's
  intended challenge is strict compatibility, this roadmap needs further layers for Weil--Deligne
  representations, local Langlands parameters, and equality at every finite place; that is not a
  renaming of the current summit.
...
`(𝔤,K)`-cohomology, automorphic representations, or compatible systems.

The [reductive-groups roadmap](../ReductiveGroups/README.md) is the shared source for affine group
schemes, Hopf algebras, algebraic representations, and Lie algebras. This roadmap consumes its
Layers 0--2 only when doing so gives the concrete `Dˣ` construction needed here. It must not create
a second incompatible representation/comodule dictionary. The open Mathlib work on
[affine analytification](https://github.com/leanprover-community/mathlib4/pull/34626) remains
important for the general theory but is not on this summit's critical path. The merged convolution
API in [mathlib4#39281](https://github.com/leanprover-community/mathlib4/pull/39281), the quaternion
reorganization in [mathlib4#41538](https://github.com/leanprover-community/mathlib4/pull/41538), and
the finite-dimensional general-linear-group work in
[mathlib4#40380](https://github.com/leanprover-community/mathlib4/pull/40380) must be checked before
the relevant Tau Ceti PRs are opened.
...
| [Reductive groups, Layer 1](../ReductiveGroups/README.md#layer-1-representations--comodules): finite-dimensional comodules and algebraic representations | algebraic coefficient representations of `Dˣ` | finite-separable Weil restriction and the comparison of its representations with the product over embeddings |
...
Finite-separable Weil restriction is a newly exposed cross-roadmap seam. The reductive-groups roadmap
places relative theory over a general base in its long horizon, which is too large a prerequisite for
this summit. This roadmap therefore builds the restricted-scalar group `Res_{F/ℚ}(Dˣ)` for finite
separable `F/ℚ`, proves its functor-of-points and base-change-to-embeddings formulas using the
three-way dictionary, and contributes the general construction back to the reductive-groups API.
...
| 1 | Layer 0; reductive-groups Layers 0--1 where landed; Mathlib adèles and Haar topology | topology on finite-dimensional scalar extensions, the idelic restricted-product topology (not the adèle-subspace topology), noncommutative orders and their completions, compact-open unit groups and congruence bases, finite-separable Weil restriction, concrete parabolics of `Dˣ` |
| 2 | Layer 1 topological groups; Mathlib invariant submodules | locally profinite interfaces, smooth-representation category and exact operations, smooth/compact induction function spaces, smooth dual, restricted tensor products and the local-factorization theorem |
...
| 5 | Layers 1, 3, 4; reductive-groups Layer 1; Mathlib chain complexes and alternating maps | full Chevalley--Eilenberg differential and signs, relative cochains for disconnected `K`, coefficient-system base change/duals, cohomology and Künneth comparisons |
...
```text
0 conventions and quaternion algebras
        │
        ├── 1 adelic, local, and restricted-scalar groups
        │                                  │
        │                                  └── 2 smooth/admissible representations
        │                                  │
        │                                  ├── 3 archimedean (𝔤,K)-modules
        │                                  │          │
        │                                  └── 4 automorphic spectrum
        │                                             │
        │                                  5 cohomological predicate
        │                                             │
        └── 6 spherical Hecke data and rationality ───┤
                                                     │
7 local Galois theory ── 8 weakly compatible systems ┤
                                                     │
                                      9 attached Galois system
```

Layers 1--6 may advance in parallel after Layer 0. Layers 7--8 are independent of automorphic
forms. Layer 9 starts only when the four incoming interfaces have executable examples and stable
names.
...
rank four. Establish the complete elementary API rather than tying the class to one use:
...
Coordinate the concrete comparison work with the active Mathlib quaternion PRs. The intrinsic class
...
The acceptance test is the classical normalized eigenform over `ℚ`: the abstract local polynomial
must reduce to `X²-a_ℓX+ℓ^{k-1}χ(ℓ)` under the documented comparison. This test detects inverses,
missing norm factors, and half-Tate shifts early.

## Layer 1: `Dˣ` locally, adelically, and by restriction of scalars
...
- the finite-separable Weil restriction `Res_{F/ℚ}(Dˣ)`, its functor-of-points formula, and after
  base change to a splitting field its comparison with the product indexed by embeddings of `F`;
...
- the global group `Dˣ(𝔸_F)`, algebraically identified with the units of the adelic scalar extension
  ...
  topology from the adèle algebra; its finite and infinite factors and the resulting topological-
  group equivalence with the restricted product of the local `D_vˣ`;
...
use its units to construct the Lie group. Prove that the result is independent of auxiliary norms
and agrees with `GL₂(ℝ)` at split real places and `ℍˣ` at ramified real places. This concrete path is
the reason general scheme analytification is not a blocker.
...
Build a reusable theory for a locally profinite group `G` over a characteristic-zero field `k`:
...
- the smooth contragredient (smooth vectors in the algebraic dual), central characters, twists, and
  ...
  place;
- local components and the factorization theorem identifying an irreducible admissible global
  finite-adèlic representation with a restricted tensor product, unique up to the standard
  isomorphisms; record unramifiedness at almost every place as part of this interface.
...
For each infinite place `v`, construct the complexified Lie algebra `𝔤_v` of `D_vˣ`, choose a
maximal compact subgroup `K_v`, and prove independence under conjugacy. Develop Harish--Chandra
...
baking those simplifications into the general definition. In the split case prove that the concrete
constant term is the usual upper-unipotent integral and is independent of the chosen splitting.
...
An automorphic representation is an isomorphism class of irreducible admissible subquotients of this
spectrum, equipped with its local components. Keep the inclusion/subquotient witness: later theorems
need to transport Hecke eigenvalues and cohomology along it. Prove independence of the chosen model.

**SIGN-OFF A occurs here.** If the summit is to include noncuspidal discrete or Eisenstein
constituents, introduce those spectra distinctly and state exactly which one-dimensional/reducible
cases the target statement covers. Do not silently broaden `IsAutomorphic`.
...
- finite-dimensional algebraic representations of `Res_{F/ℚ} Dˣ`, combining Layer 1's finite-
  separable Weil restriction with the algebraic-representation/comodule API of the reductive-groups
  roadmap; include scalar extension to `ℂ`, duals, tensor products, and comparison of algebraic and
  differentiated analytic actions;
...
**SIGN-OFF B occurs here.** Pin whether `W` or its dual appears, how the center is treated in
`(𝔤,K)`, and the C-algebraic/L-algebraic twist. The finite-place polynomial in Layer 6 and the
Galois representation in Layer 9 must be adjusted together if this convention changes.
...
content. It is closed when the classical eigenform and totally definite examples produce the exact
displayed polynomial and a change of local splitting or Haar presentation provably leaves it
unchanged.
...
## Layer 8: weakly compatible systems

Define a `d`-dimensional family over a number field `E` as representations over `Q̄_p` indexed by a
prime `p` and a field embedding `φ : E → Q̄_p`. A weak-compatibility witness consists of:
...
is that the `p`-adic cyclotomic characters form a rank-one compatible system with the stated
...
**SIGN-OFF C occurs here.** “Strict compatibility” is a different structure, requiring local
Weil--Deligne representations and a common local parameter even at bad places. It is not part of the
weak predicate and must not be claimed by the summit theorem.
...
in the weak good-place form. -/
theorem exists_weaklyCompatibleSystem
    ...
    ∃ (E : Type*) (_ : Field E) (_ : NumberField E)
      ...
      ρ.IsSemisimple ∧ ρ.IsWeaklyCompatibleWith rationality.heckePolynomials := by
  sorry
```
...
- the precise automorphic spectrum and the cuspidality decision;
...
2. A classical normalized cuspidal eigenform over `ℚ` yields the polynomial
   `X²-a_ℓX+ℓ^{k-1}χ(ℓ)` under the comparison maps.
3. A totally definite, finite-unramified quaternion algebra at parallel weight two and trivial
   central character recovers the migrated FLT automorphic-form and good `T_v` interfaces; its
   conjectural polynomial has trace `T_v` and determinant `N(v)`.
4. The relative-cohomology predicate computes the expected algebraic weights for both
   `GL₂(ℝ)` and `ℍˣ`.
5. The cyclotomic characters satisfy the rank-one weak-compatibility predicate.
6. Enlarging the coefficient field or bad set does not change the represented compatible system.
7. No Tau Ceti file imports `FLT`; a repository check enforces this.
...
This table identifies candidate source material as of 2026-07-31. It does not authorize copying and
does not prescribe Tau Ceti's final file structure.

| FLT source | Material to generalize or migrate | Destination layer |
|---|---|---|
| `FLT/Mathlib/Algebra/IsQuaternionAlgebra.lean` | central-simple rank-four class and elementary API | 0 |
...
| `FLT/Deformations/RepresentationTheory/GaloisRepFamily.lean` | embedding-indexed weak compatibility | 8 |
...
authors, reconciles naming with current Mathlib, adds general tests in Tau Ceti, and leaves an FLT
follow-up issue. Once an API lands in Tau Ceti, the corresponding FLT PR imports Tau Ceti and proves
comparison lemmas with the specialized FLT definitions before deleting duplication.

## References

- R. Taylor, *On Galois representations associated to Hilbert modular forms*, Invent. Math. 98
  (1989), 265--280: the good-place `T_v,S_v` formulation and arithmetic target.
...
- The motivating [Lean Zulip discussion of Conjecture
  B](https://leanprover.zulipchat.com/#narrow/channel/416277-FLT/topic/Main.20blockers.20for.20a.20full.20proof.20of.20FLT.2E/near/613821903).
