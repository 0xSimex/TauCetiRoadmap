#!/usr/bin/env python3
"""Re-verify an archived `Discharged.lean` against the revisions it names.

A discharge record states each target of a roadmap and closes it with the Tau Ceti declaration
that realizes it, so that a completion claim is checked by the Lean kernel rather than asserted
in prose. Once its roadmap is archived under `Completed/`, the record is finished: closing a
...
def records() -> list[pathlib.Path]:
    return sorted((ROOT / "Completed").rglob("Discharged.lean"))
